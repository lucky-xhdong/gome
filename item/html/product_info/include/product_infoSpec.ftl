<#-- 主信息：具体参数模板   -->
<ul class="specbox">
    <li class="corr">如果您发现商品信息不准确，<a href="javascript:;" class="j-corr">欢迎纠错</a></li>
<#if page.prdSpec?exists && (page.prdSpec?size>0)>
    <#assign prdSpec=page.prdSpec!>
    <li class="pedcaption">规格</li>
    <#list prdSpec as spec>
        <li class="spectit">${spec.modeTit!}</li>
        <#list spec.modeData as spinfo>
            <li><span class="specinfo">${spinfo.modeTxt!}<#if spinfo.knowledgeClub?? && spinfo.knowledgeClub !=""><i knowledgeClub="${spinfo.knowledgeClub!}"></i></#if></span><span>${spinfo.modeInfo!}</span></li>
        </#list>
    </#list>
</#if>
</ul>
